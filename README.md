...
- **Live sample grid** — 6 archetypes × 3 zooms, auto-populates from
  ...
  `untested`. Counter shows `<filled>/<total> tested`.
...
Honest current state: every preset is `curation-only`. See `/process` for the
full breakdown of what's done versus pending.
...
2. **Click a preset card** — good picks are *Shan Shui* (the v1.1 add),
   *Paris 1924*, *Indigo Cyanotype*, *Acid Topo*. The detail page shows
   lineage references, locked palette + hex codes, the prompt template (with
   copy button), negative hints, and the live 6 × 3 sample grid (currently
   `0/18 tested` for every preset — auto-populates as samples drop in).
